...
crsdata = pd.read_csv('Data/weathertrafficandflightswithCRS-2018.csv')
...
layout = html.Div([
                html.Div( 
                 [html.H3([html.Span('DELAY PREDICTIONS',className="main-heading-primary-in"),
                           html.Span('DELAY PREDICTIONS WITH FEATURE CONTROL',className="main-heading-secondary")]
...
html.Div([
				...
				html.Div([html.H3('Date Range:'),
					dcc.DatePickerRange(id='my_date_picker-DD20',
				...
				html.Div([
					html.Button(id='submit-button-DD20',
								n_clicks = 0,
								children = 'Submit',
				...
				html.Div([dcc.Graph(id='my_graph-DD20',
    ...
    value='S'
)],style={'margin-top': '-380px','margin-left':'40px'}),
...
@app.callback(
                Output('my_ticker_symbol-DD22', 'options'),
                [Input('my_ticker_symbol-DD23', 'value')]
)
def set_destination_options_DD2(selected_origin):
...
@app.callback(
                Output('my_ticker_symbol-DD21', 'options'),
                [Input('my_ticker_symbol-DD23', 'value'),
                Input('my_ticker_symbol-DD22', 'value')]
)
def set_carrier_options_DD2(selected_carrier, selected_origin):
...
@app.callback(
                Output('my_ticker_symbol-DD20', 'options'),
                [Input('my_ticker_symbol-DD23', 'value'),
                Input('my_ticker_symbol-DD22', 'value'),
                Input('my_ticker_symbol-DD21', 'value')]
)
def set_flight_options_DD2(selected_carrier, selected_origin, selected_dest):
                new_data = datadash[datadash['UNIQUE_CARRIER'] == selected_carrier]
                new_data = new_data[new_data['ORIGIN'] == selected_origin]
...
@app.callback(Output('my_graph-DD20', 'figure'),
				[Input('submit-button-DD20', 'n_clicks'),
                 Input('my_ticker_symbol-DD23', 'value'),
                 Input('my_ticker_symbol-DD22', 'value'),
                 Input('my_ticker_symbol-DD21', 'value'),
                 Input('my_ticker_symbol-DD20', 'value'),
				Input('my_date_picker-DD20', 'start_date'),
                Input('my_date_picker-DD20', 'end_date'),
                Input('my_radio-DD20', 'value')])
def update_graph_DD2(n_clicks,airline,origin,dest,flnum,startdate,enddate,radio):
    print("+++++++++++++++++++++++++++++++",radio)
    print("++++++++++++++",app)
...
'DEST_PHL','DEST_SFO']]
        ...
        hoverList = []
        weatherSeries = pd.Series(L_weather)
        print(weatherSeries.value_counts())
        if(radio=='S'):
            for j in range(len(L)):
                stra = "Delay :"+str(round(L[j],2))+"minutes"+"<br>"+"STA :"+str(crsdata[j])+"<br>"+"ETA :"+str(crsdata[j]+datetime.timedelta(minutes = int(L[j])))
                hoverList.append(stra)
        elif(radio=='CARRIER_DELAY'):
            for j in range(len(L)):
                k = L_carrier[j]*L[j]
                L[j] = L[j]-k
                stra = "Delay :"+str(round(L[j],2))+"minutes"+"<br>"+" with Delay decrement of "+str(k)+" minutes"+"<br>"+"STA :"+str(crsdata[j])+"<br>"+"ETA :"+str(crsdata[j]+datetime.timedelta(minutes = int(L[j])))
                hoverList.append(stra)
            ...
            for j in range(len(L)):
                k = L_weather[j]*L[j]
                L[j] = L[j]-k
                stra = "Delay :"+str(round(L[j],2))+"minutes"+"<br>"+" with Delay decremet of "+str(k)+" minutes"+"<br>"+"STA :"+str(crsdata[j])+"<br>"+"ETA :"+str(crsdata[j]+datetime.timedelta(minutes = int(L[j])))
                hoverList.append(stra)
