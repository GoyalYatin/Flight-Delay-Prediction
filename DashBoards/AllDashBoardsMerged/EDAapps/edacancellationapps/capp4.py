...
layout = html.Div([
        html.Div( 
                 [html.H3([html.Span('EDA-CANCELLATION DASHBOARD',className="main-heading-primary-in"),
                           html.Span('AIRLINE ROUTE-WISE CANCELLATIONS',className="main-heading-secondary")]
...
html.Div([
				...
				], style={'display': 'inline-block', 'verticalAlign':'top', 'width': '30%','margin-top':'-30px','margin-left':'100px'}),
                
				html.Div([html.H3('Destination Airport:', style={'paddingRight': '30px'}),
				dcc.Dropdown(
						  id='my_ticker_symbol-ce42',
						   # value = ['SPY'], 
				...
				html.Div([html.H3('Date Range:'),
					dcc.DatePickerRange(id='my_date_picker-ce4',
				...
				html.Div([
					html.Button(id='submit-button-ce4',
								n_clicks = 0,
								children = 'Submit',
				...
				html.Div([
                 dcc.Graph(id='my_graph_1-ce4',
							
				),],style={'width': '85%', 'display': 'inline-block', 'padding': '0 20','margin-left': '220px','margin-top': '-35px'}),	

html.Div(id = 'textonly-ce4',children='Cancellation Distribution', style={
        'textAlign': 'center',
...
@app.callback(
                Output('my_ticker_symbol-ce42', 'options'),
                [Input('my_ticker_symbol-ce43', 'value')]
)
def set_flightNumber_options_ce4(origin):
                new_data = data[data['ORIGIN'] == origin]
                return [{'label' : i, 'value' : i} for i in new_data['DEST'].unique()]

@app.callback(
                Output('my_ticker_symbol-ce4', 'options'),
                [Input('my_ticker_symbol-ce43', 'value'),
                Input('my_ticker_symbol-ce42', 'value')]
)
def set_destination_options_ce4(origin, dest):
                new_data = data[data['ORIGIN'] == origin]
                new_data = new_data[new_data['DEST'] == dest]
                return [{'label' : i, 'value' : i} for i in new_data['UNIQUE_CARRIER'].unique()]


@app.callback(Output('my_graph_1-ce4', 'figure'),
				[Input('submit-button-ce4', 'n_clicks'),
                 Input('my_ticker_symbol-ce43', 'value'),
                 Input('my_ticker_symbol-ce42', 'value'),
				Input('my_ticker_symbol-ce4', 'value'),  
				Input('my_date_picker-ce4', 'start_date'),
                Input('my_date_picker-ce4', 'end_date')])
def update_graph_1_ce4(n_clicks,origin,dest,stock_ticker,startdate,enddate):
...
@app.callback(Output('textonly-ce4','children'),
				[Input('submit-button-ce4', 'n_clicks'),
                 Input('my_ticker_symbol-ce43', 'value'),
                 Input('my_ticker_symbol-ce42', 'value'),
				Input('my_ticker_symbol-ce4', 'value'),  
				Input('my_date_picker-ce4', 'start_date'),
                Input('my_date_picker-ce4', 'end_date')])
def update_text_1_ce4(n_clicks,origin,dest,stock_ticker,startdate,enddate):
