...
layout = html.Div([html.Div( 
                 [html.H3([html.Span('EDA-CANCELLATION DASHBOARD',className="main-heading-primary-in"),
                           html.Span('FLIGHT CANCELLATIONS WITH REASON DISTRIBUTION',className="main-heading-secondary")]
...
html.Div([
				...
				], style={'display': 'inline-block', 'verticalAlign':'top', 'width': '30%','margin-left':'120px'}),
                
				html.Div([html.H3('Date Range:'),
					dcc.DatePickerRange(id='my_date_picker-ce1',
				...
				html.Div([
					html.Button(id='submit-button-ce1',
								n_clicks = 0,
								children = 'Submit',
                ...
                html.Div([
                dcc.Graph(id='my_graph_1-ce1',
							
				),],style={'width': '85%', 'display': 'inline-block', 'padding': '0 20','margin-left': '220px','margin-top': '-45px'}),
	
html.Div(id = 'textonly-ce1',children='Cancellation Distribution', style={
        'textAlign': 'center',
...
@app.callback(
                Output('my_ticker_symbol-ce12', 'options'),
                [Input('my_ticker_symbol-ce13', 'value')]
)
def set_origin_options_ce1(selected_airline):
...
@app.callback(
                Output('my_ticker_symbol-ce11', 'options'),
                [Input('my_ticker_symbol-ce13', 'value'),
                Input('my_ticker_symbol-ce12', 'value')]
)
def set_destination_options_ce1(selected_airline, origin_airport):
                new_data = data[data['UNIQUE_CARRIER'] == selected_airline]
...
@app.callback(
                dash.dependencies.Output('my_ticker_symbol-ce1', 'options'),
                [dash.dependencies.Input('my_ticker_symbol-ce13', 'value'),
                dash.dependencies.Input('my_ticker_symbol-ce12', 'value'),
                dash.dependencies.Input('my_ticker_symbol-ce11', 'value')]
)
def set_flightno_options_ce1(selected_airline, origin_airport, destination_airport):
                new_data = data[data['UNIQUE_CARRIER'] == selected_airline]
                new_data = new_data[new_data['ORIGIN'] == origin_airport]
...
@app.callback(Output('my_graph_1-ce1', 'figure'),
				[Input('submit-button-ce1', 'n_clicks'),
                 Input('my_ticker_symbol-ce13', 'value'),
                 Input('my_ticker_symbol-ce12', 'value'),
                 Input('my_ticker_symbol-ce11', 'value'),
				Input('my_ticker_symbol-ce1', 'value'),  
				Input('my_date_picker-ce1', 'start_date'),
                Input('my_date_picker-ce1', 'end_date')])
def update_graph_1_ce1(n_clicks,airline,origin,dest,stock_ticker,startdate,enddate):
    
    print("loooooooooooooooooooooooooolwaaaaaaaaaaa")
...
@app.callback(Output('textonly-ce1', 'children'),
				...
				Input('my_date_picker-ce1', 'start_date'),
                Input('my_date_picker-ce1', 'end_date')])
def update_carrier_1_ce1(n_clicks,airline,origin,dest,stock_ticker,startdate,enddate):
