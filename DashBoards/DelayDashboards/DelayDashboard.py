...
def update_graph(n_clicks,airline,origin,dest,flnum,startdate,enddate):
...
'DEST_PHL','DEST_SFO']]
        ...
        #below code populates the hover list for delay reason percentage distribution
        for j in range(len(L)):
            if(L[j]>0):
                a = str(L[j])+" "+"Carrier Delay "+str(L_carrier[j])+"%"+" "+"Late Aircraft Delay "+str(L_late_aircraft[j])+"%"+" "+"NAS Delay "+str(L_nas[j])+"%"+" "+"Weather Delay "+str(L_weather[j])+"%"+" "+"Security Delay "+str(L_security[j])+"%"
