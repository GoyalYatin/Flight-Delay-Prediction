...
#below callback returns the flight number options for selected airline
@app.callback(
                Output('my_ticker_symbol', 'options'),
                [Input('my_ticker_symbol-3', 'value')]
)
def set_flightNumber_options(selected_airline):
...
def update_graph(n_clicks,airline,stock_ticker,startdate,enddate):
...
@app.callback(Output('textonly', 'children'),
				...
				Input('my_date_picker', 'start_date'),
                Input('my_date_picker', 'end_date')])
def update_text(n_clicks,airline,stock_ticker,startdate,enddate):
